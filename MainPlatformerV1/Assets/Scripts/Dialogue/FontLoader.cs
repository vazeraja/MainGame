...
namespace MainGame {
    ...
    public static class FontLoader {
        ...
        private static readonly char[] Chars = "!\"#$%&'()*+,-./0123456789:;<=>?@abcdefghijklmnopqrstuvwxyz[\\]^_ABCDEFGHIJKLMNOPQRSTUVWXYZ{|}~".ToCharArray();
        private static readonly List<Texture2D> LoadedFontResources = new List<Texture2D>(); //This is to keep track of all the loaded fonts and only load them once
        ...
        public static List<Dictionary<char, CharData>> ReloadFontResources(params Texture2D[] characterSheet) 
            => characterSheet != null && characterSheet.Any() ? characterSheet.Select(ReloadFontResource).ToList() : new List<Dictionary<char, CharData>>();
        ...
        /// </summary>
        private static bool IsFontLoaded(Texture2D characterSheet) => LoadedFontResources.Contains(characterSheet);

        /// <summary>
        /// Generates font dictionary by performing a vertical scan on each font sprite and stores character width
        /// </summary>
        private static Dictionary<char, CharData> GenerateCharFontDictionary(Texture2D characterSheet, int spriteSize, IReadOnlyList<Sprite> characterSprites) {
            var height = characterSheet.height; // We might need this if we ever use a text image that is on more than one line
            ...
            for (var texCoordY = height - spriteSize; texCoordY >= 0 && charIndex < Chars.Length; texCoordY -= spriteSize) {
